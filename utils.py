...
def update_dataset_from_model(dataset, model, class_thresh=0.5):
    ...
    try:
        ...
        pos_ind = np.where(np.array([1.0, 0.0]) == dataset_pred.favorable_label)[0][0]
...
def update_dataset_from_model2(dataset, dataset2, model, class_thresh=0.5):
    """
    Returns a copy of `dataset` with updated scores and labels predicted by `model`
    Args:
        dataset (StandardDataset): A StandardDataset
    ...
    # Create copy of dataset
    dataset_pred = dataset.copy(deepcopy=True)
    dataset_pred2 = dataset2.copy(deepcopy=True)
    ...
    dataset_pred2.scores = y_pred_prob.reshape(-1,1)
    dataset_pred2.labels = y_pred

    return dataset_pred2
...
def update_german_dataset_from_multiple_protected_attributes(dataset):
    """
...
def update_hc_dataset_from_multiple_protected_attributes(dataset):
    """
    Returns a copy of the german dataset with updated protected attribute based on an OR summation of `age` and `sex`. This doesn't modify `dataset`.
    Args:
        dataset (StandardDataset): The hc dataset in the aif360 format, containing only one protected attribute.
    Returns:
        StandardDataset: Another hc dataset whose protected attribute is an OR summation of `age` and `sex`.
    """
    # create new dataset and get the shape of the current protected attr
    dataset_updated = dataset.copy(deepcopy=True)
    shape_protected_attribute = dataset_updated.protected_attributes.shape
    ...
    # load the german dataset with both age and sex and protected attr
    dataset_homecredit2 = pd.read_csv('data/homecredit.csv', header=0, sep=',', quotechar='"')

    dataset_homecredit2 = utils.preprocess_homecredit(dataset_homecredit2)

    dataset_homecredit_multi_protected_attr = utils.convert_to_standard_dataset(
            df=dataset_homecredit,
            target_label_name='TARGET',
            sensitive_attribute=['AGE', 'CODE_GENDER'],
            priviledged_classes=[lambda x: x >= 25, 1],
...
def compute_metrics_mult(dataset, dataset2, model, threshold):
    """
    Computes a pletora of perfomance metrics of a (fairness) model on a dataset over an specific threshold.
    Args:
        dataset (StandardDataset): dataset with classes in the aif360 format.
    ...
    privileged_groups, unprivileged_groups = get_privileged_groups(dataset2)
    ...
    dataset_pred = update_dataset_from_scores(dataset2, y_val_pred_prob[:, pos_ind], threshold)
...
def compute_metrics_from_scores_mult(dataset_true, dataset_pred, dataset2, threshold):
    """
    Computes a pletora of perfomance metrics of a (fairness) model using only the predicted scores over an specific threshold.
    This function doesn't require the model argument as in other `compute_metrics` functions
    Relevant for Platt scaling
    ...
    dataset_pred2 = dataset2.copy(deepcopy = True)
    dataset_pred2.labels[fav_inds] = dataset_true.favorable_label
    dataset_pred2.labels[~fav_inds] = dataset_true.unfavorable_label

    metric = ClassificationMetric(dataset2,
                                  dataset_pred2,
                                  unprivileged_groups=unprivileged_groups,
                                  privileged_groups=privileged_groups)
...
##############################################
# Threshold sweep when there is an or variable
##############################################

def undo_or(dataset, original_dataset):
    label_names0 = original_dataset.label_names
    favorable_label0 = original_dataset.favorable_label
    unfavorable_label0 = original_dataset.unfavorable_label
    protected_attribute_names0 = original_dataset.protected_attribute_names
    unprivileged_protected_attributes0 = original_dataset.unprivileged_protected_attributes
    privileged_protected_attributes0 = original_dataset.privileged_protected_attributes
    metadata0 = original_dataset.metadata
    
    labels = dataset.labels
    features = original_dataset.features

    df0 = np.append(features, labels, axis = 1)
    names = np.append(original_dataset.feature_names, label_names0)
    df0 = pd.DataFrame(df0, columns = names)

    new_data = BinaryLabelDataset(df = df0,
                              label_names = label_names0, favorable_label = favorable_label0,
                              unfavorable_label = unfavorable_label0, protected_attribute_names = protected_attribute_names0,
                              unprivileged_protected_attributes = unprivileged_protected_attributes0,
                              privileged_protected_attributes = privileged_protected_attributes0, 
                              metadata = metadata0)
    return new_data

def metrics_threshold_sweep_mult(dataset, dataset2, model, thresh_arr):
    """
    Computes a pletora of perfomance metrics of a (fairness) model on a dataset over an array of score thresholds.
    Args:
        dataset1 (StandardDataset): dataset with classes in the aif360 format (one sensible attribute).
        dataset2 (StandardDataset): dataset with classes in the aif360 format (multiple sensible variables).
        model: aif360 or sklearn trained classifier.
        thresh_arr (list): List of thresholds to sweep the metrics through.
    Returns:
        dict: Dictionary of metrics where keys are metrics and values are arrays with the metric value over
    ...
    FPR, TPR, _ = roc_curve(metric_arrs['y_true'], metric_arrs['y_pred_probs'], pos_label=dataset2.favorable_label)
    metric_arrs['auc'] = auc(FPR, TPR)

    for thresh in thresh_arr:
        dataset_pred = update_dataset_from_scores(dataset2, y_val_pred_prob[:, pos_ind], thresh)
        #dataset_pred_unor = undo_or(dataset_pred, dataset2)
...
def compute_metrics_postprocessing_mult(dataset_true, dataset2, dataset_preds, model, threshold=None, required_threshold=True,
                                   scores_or_labels='scores'):
    """
    Computes a pletora of perfomance metrics of a postprocessing model on a dataset over an array of score thresholds.
    Args:
    ...
    dataset_preds2 = dataset2.copy(deepcopy = True)
    dataset_preds2.scores = dataset_preds.scores
    ...
    if scores_or_labels == 'scores' and required_threshold is True:
        dataset_preds_trans = dataset_preds.copy()
        dataset_preds2_trans = dataset2.copy(deepcopy = True)
        dataset_preds_trans.labels = model.predict(dataset_preds, threshold).labels
        dataset_preds2_trans.labels = dataset_preds_trans.labels
    elif scores_or_labels == 'labels' and required_threshold is True:
        dataset_preds = update_dataset_from_scores(dataset_preds, dataset_preds.scores, threshold)
        dataset_preds2 = update_dataset_from_scores(dataset2, dataset_preds.scores, threshold)
        dataset_preds_trans = dataset_preds.copy()
        dataset_preds2_trans = dataset_preds2.copy()
        dataset_preds_trans.labels = model.predict(dataset_preds).labels
        dataset_pred2_trans.labels = dataset_preds_trans.labels
    elif required_threshold is False:
        metrics_best_thresh['best_threshold'] = model.classification_threshold
        dataset_preds_trans = dataset_preds.copy()
        dataset_preds2_trans = dataset2.copy()
        dataset_preds_trans.labels = model.predict(dataset_preds).labels
        dataset_preds2_trans.labels = dataset_preds.labels
    else:
        raise ValueError('Invalid value for parameter scores_or_labels or required_threshold.')

    metric = ClassificationMetric(
        dataset2, dataset_preds2_trans,
        unprivileged_groups=unprivileged_groups,
        privileged_groups=privileged_groups)
...
def metrics_postprocessing_threshold_sweep_mult(dataset_true, dataset2, dataset_preds, model, thresh_arr, scores_or_labels='scores'):
    """
    Computes a pletora of perfomance metrics of a postprocessing model on a dataset over an array of score thresholds.
    Args:
        dataset_true (StandardDataset): dataset with classes in the aif360 format.
    ...
    for t in thresh_arr:
        if scores_or_labels == 'scores':
            ...
            dataset_preds_trans2 = dataset2.copy()
            dataset_preds_trans2.labels = dataset_preds_trans.labels
        elif scores_or_labels == 'labels':
            dataset_preds = update_dataset_from_scores(dataset_preds, dataset_preds.scores, t)
            dataset_preds_trans = dataset_preds.copy()
            dataset_preds_trans.labels = model.predict(dataset_preds).labels
...
def metrics_postprocessing_threshold_sweep_from_scores_mult(dataset_true, dataset2, dataset_preds, thresh_arr):
    """
    Computes a pletora of perfomance metrics on a predicted dataset with scores over an array of score thresholds.
    Main difference with other `metrics_postprocessing_*` functions is that this doesn't require a model to sweep.
    Relevant only for Platt Scaling since it doesn't use a single model (there is a model for each sensitive group)
    ...
    for thresh in thresh_arr:
        ...
        # create predicted dataset
        fav_inds = dataset_preds.scores > thresh
        dataset_preds_updated.labels[fav_inds] = dataset2.favorable_label
        dataset_preds_updated.labels[~fav_inds] = dataset2.unfavorable_label

        metric = ClassificationMetric(dataset2, dataset_preds_updated,
                                      unprivileged_groups=unprivileged_groups,
                                      privileged_groups=privileged_groups)
...
def preprocess_homecredit_mult(df):
    """
    Args:
        df: trainset from the homecredit data competition (https://www.kaggle.com/c/home-credit-default-risk)
    Returns:
    ...
    df.loc[df.loc[:,'CODE_GENDER'] == 'M', 'PROT_ATTR'] = 1.0
    df.loc[df.loc[:,'AGE'] >= 25, 'PROT_ATTR'] = 1.0 

    # Drop SK_ID_CURR: id column; DAYS_BIRTH: re-formated as AGE
    df = df.drop(columns=['SK_ID_CURR', 'DAYS_BIRTH'])

    encoding_vars = ['NAME_CONTRACT_TYPE', 'CODE_GENDER', 'FLAG_OWN_CAR', 'FLAG_OWN_REALTY', 'NAME_TYPE_SUITE',
                     'NAME_INCOME_TYPE', 'NAME_EDUCATION_TYPE', 'NAME_FAMILY_STATUS', 'NAME_HOUSING_TYPE',
                     'OCCUPATION_TYPE', 'WEEKDAY_APPR_PROCESS_START', 'ORGANIZATION_TYPE', 'FONDKAPREMONT_MODE',
                     'HOUSETYPE_MODE', 'WALLSMATERIAL_MODE', 'EMERGENCYSTATE_MODE']
